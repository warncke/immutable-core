...
function createMethodWrapperFunction (moduleMeta, methodMeta) {
     ...
     * @throws {Error}
     */
    var methodWrapperFunction = function methodWrapperFunction (args, bindType, bindSignature) {
    ...
    return ret.then(res => {
        ...
        return Promise.all(
            ...
            _.map(methodMeta.after, method => {
                return (method)(args, 'after', methodMeta.signature)
            })
        )
...
function runAfterDetach (args, methodMeta, ret) {
    ...
    ret.then(res => {
        ...
        _.each(methodMeta.afterDetach, method => {
            // call method
            (method)(args, 'afterDetach', methodMeta.signature)
            // log errors
            .catch(err => {
                // require log client
...
function runBeforeDetach (args, methodMeta) {
    ...
    _.each(methodMeta.beforeDetach, method => {
        // call method
        (method)(args, 'beforeDetach', methodMeta.signature)
        // log errors
        .catch(err => {
            // require log client
            if (!defined(methodMeta.logClient)) {
...
function runBefore (args, methodMeta) {
    ...
    return Promise.all(
        ...
        _.map(methodMeta.before, method => {
            return (method)(args, 'before', methodMeta.signature)
        })
    )
