...
function ImmutableCoreModule (moduleName, options) {
    ...
    var data
    // create getter/setter for global meta data on meta
    Object.defineProperty(this.meta, 'data', {
        get: () => data,
        set: newData => {
            // validate data - throws on error
            newData = validateSetData(this, newData)
            // set data if defined
            if (defined(newData)) {
                data = newData
            }
    ...
    // create getter/setter for global meta data on instance
...
/**
 * @function validateSetData
 *
 * validate and set data
 *
 * @param {ImmutableCoreModule} module
 ...
 * @throws {Error}
 */
function validateSetData (module, newData) {
    // require valid data
    if (typeof newData !== 'object' || newData === null) {
        throw new Error(`invalid data for ${module.meta.name}`)
    }
    // get data id
