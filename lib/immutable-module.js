...
function cache (methodName, cache) {
    ...
    if (!cache.cacheClient) {
        cache.cacheClient = moduleMeta.options.cacheClient
    }
...
function method (methodName, method, options) {
    ...
    var allowOverride = options.allowOverride !== undefined ? options.allowOverride : moduleMeta.options.allowOverride
    // throw exception if method already defined
    if (module[methodName] && !allowOverride) {
    ...
    var methodMeta = {
        // automock function
        automock: options.automock || moduleMeta.options.automock,
        // default arg values
    ...
    this[methodName] = methodMeta.automock
        ...
        ? methodMeta.automock( createMethodWrapperFunction(moduleMeta, methodMeta) )
        // create regular method with no mock
        : createMethodWrapperFunction(moduleMeta, methodMeta)
...
function createMethodWrapperFunction (moduleMeta, methodMeta) {
     ...
     * @throws {Error}
     */
    var methodWrapperFunction = async function methodWrapperFunction (args) {
...
function getValidArgs (args, methodMeta) {
    ...
    if (!args || typeof args !== 'object') {
        ...
        if (methodMeta.strictArgs) {
            return Promise.reject(new Error(methodMeta.signature+': invalid args for - typeof args: '+typeof args))
        }
    ...
    if (!args.session || typeof args.session !== 'object') {
        // throw exception in strict mode
        if (methodMeta.strictArgs) {
            return Promise.reject(new Error(methodMeta.signature+': invalid session for - typeof session: '+typeof args.session))
        }
        // otherwise create new object for args
        else {
            args.session = {}
