...
function getFunction (functionName, functionObj, options) {
    ...
    if (!functionObj) {
        // require function to be defiend
        assert.ok(immutable.functions[functionName], 'function error: function not found '+functionName)
        // return function
    ...
    if (immutable.functions[functionName] && !options.allowOverride) {
...
function getModule (name, methods, options) {
    ...
    if (immutable.modules[name] && !options.allowOverride) {
...
function hasFunction (functionName) {
    // check if function exists
    return immutable.functions[functionName] ? true : false
}
...
function setData (moduleName, data) {
    ...
    if (typeof data !== 'object' || data === null) {
        throw new Error(`invalid data for ${moduleName}`)
    }
    // get module - throws error if not defined
    var module = getModule(moduleName)
    // get data id
    var dataId = stableId(data)
    // if data has not changed do not set
    if (dataId === module.meta.dataId) {
        return
    }
    // freeze data
    if (module.meta.options.freezeData) {
        deepFreeze(data)
    }
    // set data and dataId
    module.meta.data = data
    module.meta.dataId = dataId
    // log data set
    if (defined(module.meta.options.logClient)) {
        module.meta.options.logClient.log('setData', {
            data: data,
