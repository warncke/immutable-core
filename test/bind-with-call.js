...
describe('immutable-core bind with method call', function () {
    ...
    it('should call bound function', async function () {
        ...
        // bind bar with foo
        ImmutableCore.with('FooModule.foo', barModule.bar)
        // test method call
    ...
    it('should have values from before methods in args', async function () {
        // create stubs for bar and foo
        var bam = sandbox.stub().resolves()
        ...
        // bind bam with foo
        ImmutableCore.with('FooModule.foo', barModule.bam)
        // test method call
        var res = await fooModule.foo({foo: true})
        // check args
        assert.calledWithMatch(bam, {foo: true, bar: true})
    })

    it('should merge values returned by bound method into result', async function () {
        // create stubs for bar and foo
        var bar = sandbox.stub().resolves({bar: true})
        var foo = sandbox.stub().resolves({foo: true})
        // create FooModule
        var fooModule = ImmutableCore.module('FooModule', {
            foo: foo,
        })
        ...
        var res = await fooModule.foo()
        // check res
        assert.deepEqual(res, {bar: true, foo: true})
    })
    ...
    it('should have correct stack', async function () {
        ...
        assert.calledWithMatch(bar, {session: {stack: [
            'FooModule.foo',
            'BarModule.bar,with,FooModule.foo',
        ]}})
    })
